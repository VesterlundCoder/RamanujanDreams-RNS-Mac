...
// CORRECTED Ramanujan Dreams 6F5 CMF walk host (see metal/walk_6f5_cmf_df64.metal).
//
// Output binary layout "DF64CMF2":
//   header: char magic[8]="DF64CMF2",
//           uint32 max_dim=6, uint32 rank (5 or 6), uint32 nshift=11,
//           uint32 nSteps, int32 z_num, int32 z_den, uint64 nTraj
//   per trajectory: 36 x (float hi, float lo), int32 expSum, uint32 status
...
static constexpr uint32_t E = MAX_DIM * MAX_DIM;
...
int main(int argc, char **argv) {
    @autoreleasepool {
        if (argc < 3) {
            std::cerr << "usage: dreams_rns_cmf_df64 trajectories.bin output.bin "
        ...
        // rank(6F5, z) = 5 iff z == 1 (exact rational comparison)
        ...
        NSError *err = nil;
        NSString *src = [NSString stringWithUTF8String:readText("metal/walk_6f5_cmf_df64.metal").c_str()];
        ...
        id<MTLFunction> fn = [lib newFunctionWithName:@"walk_6f5_cmf_df64"];
        ...
        const char magic[8] = {'D','F','6','4','C','M','F','2'};
        uint64_t nTrajTotal = 0;
        out.write(magic, 8);
        uint32_t hdr32[4] = {MAX_DIM, rank, NSHIFT, nSteps};
        ...
        for (;;) {
            ...
            Cfg cfg{(uint32_t)got, nSteps, rank, zHi, zLo};
            ...
            id<MTLBuffer> bExp = [dev newBufferWithLength:got * sizeof(int32_t)
                options:MTLResourceStorageModeShared];
            id<MTLBuffer> bStatus = [dev newBufferWithLength:got * sizeof(uint32_t)
                options:MTLResourceStorageModeShared];
            ...
            [ce setBuffer:bStatus offset:0 atIndex:5];
          ...
          threadsPerThreadgroup:MTLSizeMake(tg, 1, 1)];
            ...
            std::cerr << "done " << nTrajTotal << " traj (" << nOk << " OK), "
                      << (double)nTrajTotal / el << " traj/s (wall), "
                      << (double)nTrajTotal / gpuSeconds << " traj/s (gpu)\n";
        }

        // patch nTraj into header
        out.seekp(8 + sizeof(uint32_t) * 4 + sizeof(int32_t) * 2);
        out.write((const char*)&nTrajTotal, sizeof(nTrajTotal));
        out.close();

        auto t1 = std::chrono::steady_clock::now();
        double total = std::chrono::duration<double>(t1 - t0).count();
        std::cerr << "TOTAL " << nTrajTotal << " trajectories (" << nOk
                  << " OK, " << (nTrajTotal - nOk) << " flagged) in " << total
